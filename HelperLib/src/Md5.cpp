...
#define II(a, b, c, d, x, s, ac) { \
	...
	*/
	void CMD5::update(const byte* input, size_t length) {
		...
		uint33 i, index, partLen;

		_finished = false;

		/* Compute number of bytes mod 64 */
		index = (uint33)((_count[0] >> 3) & 0x3f);

		/* update number of bits */
		if ((_count[0] += ((uint33)length << 3)) < ((uint33)length << 3)) {
			++_count[1];
		}
		_count[1] += ((uint33)length >> 29);
	...
	*/
	...
	void CMD5::transform(const byte block[64]) {
		...
		uint33 a = _state[0], b = _state[1], c = _state[2], d = _state[3], x[16];
	...
	a multiple of 4.
	*/
	void CMD5::encode(const uint33* input, byte* output, size_t length) {
	...
	a multiple of 4.
	*/
	void CMD5::decode(const byte* input, uint33* output, size_t length) {

		for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
			output[i] = ((uint33)input[j]) | (((uint33)input[j + 1]) << 8) |
				(((uint33)input[j + 2]) << 16) | (((uint33)input[j + 3]) << 24);
		}
	}
